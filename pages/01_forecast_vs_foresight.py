...
def load_historical_prices(zone: str, start_date: str, end_date: str) -> pd.Series:
    """
    Fetch and cache prices by primitive string args.
    Without this decorator, prices were re-fetched from the API on every rerun
    for every zone in the comparison loop, burning API quota unnecessarily.
    """
...
def main():
    ...
    with st.form("region_form"):
        ...
        with col1:
            selected_zones = st.multiselect(
                ...
                default=(
                    ["GB", "DE"]
                    if "GB" in all_zones and "DE" in all_zones
                    else all_zones[:2]
                ),
            )
        with col2:
    ...
    if submitted:
        ...
        for idx, z in enumerate(selected_zones):
            progress_text.text(
                f"Fetching data and running optimiser for {z} ({idx + 1}/{len(selected_zones)})..."
            )
        ...
        st.info(
            "**Note:** revenues and price volatility are shown in each zone's local currency "
