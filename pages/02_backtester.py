...
def main():
    st.title("📈 Backtester")
    st.markdown(
        "Run the optimiser over an extended historical period to measure long-term value creation."
    )
    render_sidebar()
    ...
    with st.form("backtest_form"):
        ...
        if model_selection == "ML Model":
            st.caption(
                "⏱ ML backtests may take 1-2 minutes depending on date range — "
                "the model trains on your full history and runs iterative inference for each day."
            )
    ...
    if submitted:
        ...
        if prices.empty or sum(prices) == 0:
            ...
            import numpy as np
            index = pd.date_range(
                start=start_fetch, end=end_date_str + " 23:00", freq="h", tz="UTC"
            )
        ...
        df_fc = run_backtest(
            prices, engine_forecast_fn, battery_params,
        ...
        with chart_col1:
            ...
            fig1.add_trace(
                go.Scatter(
                    x=df_pf.index, y=df_pf["cum_profit"],
                    name="Perfect Foresight", line=dict(color="green"),
                )
            )
            fig1.add_trace(
                go.Scatter(
                    x=df_fc.index, y=df_fc["cum_profit"],
        ...
        with chart_col2:
            ...
            if num_days <= 31:
                ...
                fig2 = go.Figure(
                    data=[
                        go.Bar(
                            x=df_resampled.index.strftime("%Y-%m-%d"),
                ...
                fig2 = go.Figure(
                    data=[
                        go.Bar(
                            x=df_resampled.index.strftime("%Y-%B"),
                            y=df_resampled.values,
                        )
                    ]
                )
        ...
        with bw1:
            ...
            st.dataframe(
                sorted_days.head(5)[["date", "profit", "throughput_MWh", "active_hours", "avg_spread"]],
                use_container_width=True,
            )
        with bw2:
            st.write("Bottom 5 Days")
            st.dataframe(
                bottom_5[["date", "profit", "throughput_MWh", "active_hours", "avg_spread"]].sort_values("profit"),
                use_container_width=True,
            )
